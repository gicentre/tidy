...
var author$project$Tidy$transpose = function (listOfLists) {
	var tails = A2(elm$core$List$filterMap, elm$core$List$tail, listOfLists);
	var heads = A2(elm$core$List$filterMap, elm$core$List$head, listOfLists);
	return (!elm$core$List$length(heads)) ? _List_Nil : (_Utils_eq(
		elm$core$List$length(heads),
		elm$core$List$length(listOfLists)) ? A2(
		elm$core$List$cons,
		heads,
		author$project$Tidy$transpose(tails)) : _List_Nil);
};
	...
	function (maxRows, tbl) {
		...
		var continues = (_Utils_cmp(numTableRows, mx) > 0) ? addDividers(
			A2(
				...
				elm$core$Dict$keys(tbl))) : _List_Nil;
		var divider = addDividers(
			A2(
				elm$core$List$map,
				elm$core$Basics$always('-'),
				elm$core$Dict$keys(tbl)));
...
var elm$regex$Regex$Match = F4(
	function (match, index, number, submatches) {
		return {aw: index, ay: match, aA: number, aH: submatches};
	});
...
var author$project$Tidy$tableHead = function (tbl) {
	return (!A3(
		...
		A2(elm$core$Basics$composeL, elm$core$Basics$max, elm$core$List$length),
		0,
		elm$core$Dict$values(tbl))) ? _List_Nil : A3(
		elm$core$Dict$foldl,
		F2(
			function (k, v) {
...
var author$project$Tidy$tableTail = elm$core$Dict$map(
	elm$core$Basics$always(
...
var author$project$Tidy$melt = F4(
	function (columnName, valueName, colVars, table) {
		var emptyTable = elm$core$Dict$fromList(
			A2(
				elm$core$List$map,
				function (label) {
			...
			var meltCol = function (_n4) {
				...
				if (!_n3.$) {
					...
					return elm$core$Maybe$Just(
						_List_fromArray(
							[
								_Utils_Tuple2(columnName, colRef),
								_Utils_Tuple2(valueName, val)
							]));
				} else {
					return elm$core$Maybe$Nothing;
				}
...
var author$project$WickhamExamples$tidy6 = A4(
	...
	_List_fromArray(
		[
			...
			_Utils_Tuple2('income10-20k', '$10-20k'),
			_Utils_Tuple2('income20-30k', '$20-30k'),
			_Utils_Tuple2('income30-40k', '$30-40k'),
			_Utils_Tuple2('income40-50k', '$40-50k'),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('Examples from Wickham (2014)')
					])),
				A2(
				elm$html$Html$h2,
				_List_Nil,
				_List_fromArray(
					[
						elm$html$Html$text('Messy Table 1')
					])),
				...
				_List_fromArray(
					[
						elm$html$Html$text('tidied as Table 3')
					])),
				author$project$WickhamExamples$toHtml(
				...
				_List_fromArray(
					[
						elm$html$Html$text('Messy Table 4')
					])),
				author$project$WickhamExamples$toHtml(
				...
				_List_fromArray(
					[
						elm$html$Html$text('tidied as Table 6')
					])),
				author$project$WickhamExamples$toHtml(
				...
				_List_fromArray(
					[
						elm$html$Html$text('tidied as Table 8')
					])),
				author$project$WickhamExamples$toHtml(
...
var author$project$WickhamExamples$main = elm$browser$Browser$sandbox(
	{ax: 0, aL: author$project$WickhamExamples$update, aN: author$project$WickhamExamples$view});
